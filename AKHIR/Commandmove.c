    ...
    for (i=1;i<17;i++){
        if ((*T).arr[i].p.isdead == false){
            if (((*T).arr[i].p.type == 'K') || ((*T).arr[i].p.type == 'k')){
                king(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }
            else if (((*T).arr[i].p.type == 'Q') || ((*T).arr[i].p.type == 'q')){
                queen(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }
            else if (((*T).arr[i].p.type == 'R') || ((*T).arr[i].p.type == 'r')){
                rook(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }
            else if (((*T).arr[i].p.type == 'B') || ((*T).arr[i].p.type == 'b')){
                bishop(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }
            else if (((*T).arr[i].p.type == 'P') || ((*T).arr[i].p.type == 'p')){
                pawn(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }    
            else if (((*T).arr[i].p.type == 'H') || ((*T).arr[i].p.type == 'h')){
                horse(&(*T).arr[i].possmove, (*T).arr[i].p, B, (*T).arr[i].p.team);
            }
        }
    } 
}
...
/* 	i = findPieceIdx(*M, P);
 */
	addValidCoor(L, P.xpos, P.ypos+1, B, T);
	addValidCoor(L, P.xpos+1, P.ypos, B, T);
	addValidCoor(L, P.xpos, P.ypos-1, B, T);
	addValidCoor(L, P.xpos-1, P.ypos, B, T);
	
	addValidCoor(L, P.xpos+1 , P.ypos+1, B, T);
	addValidCoor(L, P.xpos+1 , P.ypos-1, B, T);
	addValidCoor(L, P.xpos-1, P.ypos-1, B, T);
	addValidCoor(L, P.xpos-1, P.ypos+1, B, T);
}
...
/* 	i = findPieceIdx(*M, P); */
	...
	addValidCoor(L, P.xpos-1, P.ypos+2, B, T);
	addValidCoor(L, P.xpos+1, P.ypos+2, B, T);
	addValidCoor(L, P.xpos+2, P.ypos+1, B, T);
	addValidCoor(L, P.xpos+2, P.ypos-1, B, T);
 ...
 */
	if (T == 'W') { 
        if (P.hasmoved) {
		    ...
		    addValidCoor(L, P.xpos, P.ypos+1, B, T);
		    addValidCoor(L, P.xpos, P.ypos+2, B, T);
	    }
...
void rook(List* L, piece P, board B, char T) {
